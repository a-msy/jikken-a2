...
$loop1:
	...
	nop
	slt	$t2, $v0, $t1		# i < n - 1
	beq	$t2, $zero, $exit1	# if !(i < n - 1) then goto $exit1
	nop
	addi	$t2, $v0, 1		# j = i + 1
	sw	$t2, j_loc($fp)
	sll	$v0, $v0, 2		# t2 = i << 2
...
$loop2:	
	...
	nop
	slt	$t2, $t0, $t1		# j < n
	beq	$t2, $zero, $exit2	# if !(j < n) then goto $exit2
	nop
	sll	$t0, $t0, 2		# t4 = j << 2
	lw	$t2, data_loc($fp)
	nop
	...
	nop
	slt	$t1, $t0, $t2		# t1 < t7
	beq	$t1, $zero, $L3		# if !(t1 < t7) then goto $L3
	nop
	lw	$t1, t3_loc($fp)
	nop
	sw	$t2, 0($t1)		# *t3 = t7
